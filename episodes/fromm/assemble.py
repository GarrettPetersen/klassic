...
def finish(build, timeline):
    ...
    subtitle = timeline['title'].removeprefix('Classic Krusty — ')
    card(build/'title.png', [('CLASSIC KRUSTY',350,100),('ERICH FROMM',530,61),(subtitle,630,44)])
    credits = [
    ...
    filters = [f'[0:v]trim=duration={INTRO_SECONDS},setpts=PTS-STARTPTS,fade=t=in:st=0:d=0.4,fade=t=out:st={INTRO_SECONDS-.4}:d=0.4[vtitle]']
    if cut:
        filters.extend([
            '[1:v]split[vprein][vpostin]',
            f'[vprein]trim=end={cut},setpts=PTS-STARTPTS,fade=t=out:st={cut-.25}:d=0.25[vpre]',
            f'[vpostin]trim=start={cut},setpts=PTS-STARTPTS,fade=t=in:st=0:d=0.25,fade=t=out:st={duration-cut-.5}:d=0.5[vpost]',
        ])
        video_labels = ['vpre','vtitle','vpost']
        speech_filters = [
            '[1:a]aresample=48000,aformat=channel_layouts=mono,asplit[sprein][spostin]',
            f'[sprein]atrim=end={cut},asetpts=PTS-STARTPTS[spre]',
            f'anullsrc=r=48000:cl=mono,atrim=duration={INTRO_SECONDS}[sgap]',
            f'[spostin]atrim=start={cut},asetpts=PTS-STARTPTS[spost]',
            f'[spre][sgap][spost]concat=n=3:v=0:a=1,apad=whole_dur={total}[speech]',
        ]
    else:
        filters.append(f'[1:v]setpts=PTS-STARTPTS,fade=t=out:st={duration-.5}:d=0.5[v1]')
        video_labels = ['vtitle','v1']
        speech_filters = [f'[1:a]aresample=48000,aformat=channel_layouts=mono,adelay={round(INTRO_SECONDS*1000)}:all=1,apad=whole_dur={total}[speech]']
    ...
    filters.extend([
        ''.join(f'[{label}]' for label in video_labels)+f'concat=n={len(video_labels)}:v=1:a=0[v]',
        *speech_filters,
        f'[{count}:a]asplit=2[opening][closing]',
        f'[opening]atrim=start=0:duration={intro},asetpts=PTS-STARTPTS,{tone},afade=t=in:st=0:d=0.15,afade=t=out:st={INTRO_SECONDS}:d=2,adelay={round(cut*1000)}:all=1[imusic]',
    ...
    atmosphere = timeline.get('presentation', {}).get('atmosphere')
    mix_inputs = '[speech][imusic][omusic]'
    if atmosphere:
        bed = build/atmosphere['file']
        if digest(bed) != atmosphere['sha256']:
            raise ValueError('Atmosphere changed after its mix was prepared')
        inputs.extend(['-i',bed])
        filters.append(f'[{count+1}:a]aresample=48000,aformat=channel_layouts=mono,atrim=duration={total}[bed]')
        mix_inputs += '[bed]'
    filters.append(mix_inputs+f'amix=inputs={4 if atmosphere else 3}:duration=longest:normalize=0,alimiter=limit=0.95:latency=1[a]')
    ...
    (build/'episode.srt').write_text('\n\n'.join(f"{i+1}\n{srt_time(screen_time(c['start'],cut,INTRO_SECONDS))} --> {srt_time(screen_time(c['end'],cut,INTRO_SECONDS,end=True))}\n{c['text']}" for i,c in enumerate(captions))+'\n')
    (build/'music-credit.txt').write_text(music['credit']+'\n')
    write_json(build/'episode-manifest.json',{'title_seconds':INTRO_SECONDS,'title_at_seconds':cut,'dialogue_seconds':duration,'atmosphere':atmosphere,
        ...
        'music':{'sha256':digest(music_path),'source_url':music['source_url'],'license_url':music['license_url'],
                 'intro_seconds':intro,'intro_at':cut,'outro_at':outro_at,'outro_seconds':outro},
        'script':timeline['source']['notes']})
    title = html.escape(timeline['title'])
    rows = ''.join(
        f"<tr><td><a href='#interview' data-seek='{screen_time(turn['start'],cut,INTRO_SECONDS):.6f}'>{srt_time(screen_time(turn['start'],cut,INTRO_SECONDS))}</a></td>"
        f"<td>{'Krusty' if turn['speaker']=='host' else 'Fromm'}</td>"
        f"<td>{html.escape(turn['text'])}</td><td>{html.escape(turn['provenance'])}</td></tr>"
...
<style>body{{background:#181818;color:#eee;font:17px/1.6 system-ui;max-width:1000px;margin:40px auto;padding:0 24px}}video{{width:100%}}a{{color:#ddd}}td{{padding:10px;border-bottom:1px solid #555;vertical-align:top}}table{{font-size:15px}}</style>
<h1>{title}</h1><video id="interview" controls poster="title.png" src="episode.mp4"></video>
<p>{html.escape(timeline['disclosure'])}</p><p><a href="episode.srt">Captions</a> · <a href="timeline.json">Dialogue timeline (title at {srt_time(cut)})</a></p>
<p>{html.escape(music['credit'])}</p>
<details><summary>Transcript and source notes</summary><p>{html.escape(timeline['source']['notes'])}</p>
<table><thead><tr><th>Time</th><th>Speaker</th><th>Dialogue</th><th>Provenance</th></tr></thead><tbody>{rows}</tbody></table></details>
<script>document.querySelectorAll('[data-seek]').forEach(link => link.addEventListener('click', () => {{document.getElementById('interview').currentTime = Number(link.dataset.seek);}}));</script></html>''')
    print(final)
